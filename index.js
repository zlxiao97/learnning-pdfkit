...
/** 表格绘制辅助方法 */
const drawRect = ({ offsetX = 0, offsetY = 0, column = DOC_MAX_COLUMNS, rowHeight, page }) => {
  const DOC_POSTION_X = page.startX
  const DOC_POSTION_Y = page.startY
  doc.strokeColor('red').rect(getX(DOC_POSTION_X + offsetX), getY(DOC_POSTION_Y + offsetY), getX(column / DOC_MAX_COLUMNS * DOC_WIDTH), getY(rowHeight)).stroke()
}
